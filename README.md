...
2. **导入工作流**：三种方式（「工作流 → 导入模板」）：
   ...
   工作流列表，页面打开即有数据。
3. **生成**：「生成」页选模板 → 填表单 → 生成数量（工作流含 EmptyLatentImage 时走 batch_size，否则串行多次提交）→ 生成，实时显示采样进度。表单草稿自动保存（换页/刷新不丢），完成的任务可「再来一次」原参数复刻（种子锁定），生成完成时手机震动提醒。
4. **画廊**：本站生成记录（存 SQLite，图片经后端代理实时读取 ComfyUI 的输出），详情页可下载原图或跳回「再次生成」。
5. **新装了模型/LoRA**：到「设置」页点「刷新模型列表」清掉缓存，生成页下拉即会更新。
6. **模型 / LoRA 页**：浏览和搜索 Civitai 上的模型与 LoRA（底模过滤、按下载/评分排序），详情页含底模、触发词（点击复制）、文件信息、"在 ComfyUI 里怎么用"指引；顶部可查看本机已安装文件，点击文件名直接在 Civitai 搜索。需要代理或 API Token 时在「设置 → Civitai」配置；图片经后端代理，局域网设备无需直连 Civitai。
